...
public class Course implements Serializable {
	...
	private static final int MAXSLOTS = 35;
	...
	public Course() {
		...
		setCourseName("Place Holder Course");
	...
	public void setNumSlots(int _numSlots) {
		...
		if (_numSlots < 0 || _numSlots > MAXSLOTS) {
			_numSlots = MAXSLOTS;
		}
		numSlots = _numSlots;
	}
	...
	public boolean addStudent(Student s) {
		...
		try {
			...
			// if class is full
			if (getStudentCount() >= MAXSLOTS) throw new StudentException();
	...
	public void printRoster() {
		...
		for (Student s : students) {
			if (s.getID() != 0) {
				out.println(s.toString() + " | GPA: " + s.calcGPA());
			}
		}
	}
